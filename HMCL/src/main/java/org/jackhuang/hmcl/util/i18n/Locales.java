    ...
    public static class SupportedLocale {
        ...
        public String i18n(String key, Object... formatArgs) {
            ...
            try {
                ...
                LOG.info("[i18n DIAG] locale=" + locale + " bundleLocale=" + bundle.getLocale()
                        + " key=" + key + " bundle.containsKey(key)=" + bundle.containsKey(key)
                        + " candidateLocales=" + getCandidateLocales(), e);
            } catch (IllegalFormatException e) {
                LOG.error("Illegal format string, key=" + key + ", args=" + Arrays.toString(formatArgs), e);
            }

            // Fallback: try root bundle directly (handles classloader / caching edge cases)
            try {
                ResourceBundle root = ResourceBundle.getBundle("assets.lang.I18N", Locale.ROOT,
                        DefaultResourceBundleControl.INSTANCE);
                if (root.containsKey(key)) {
                    try {
                        return String.format(root.getString(key), formatArgs);
                    } catch (IllegalFormatException e) {
                        LOG.error("Fallback root bundle also failed with format error, key=" + key + ", args=" + Arrays.toString(formatArgs), e);
                    }
                }
